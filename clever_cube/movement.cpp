...
int moveUpAndDown(RECT &r, RECT limit, int step, int id, RECT barriers[], double barriersInfo[][100], int reset) {
	...
	for(int i = 0; i < 100; i++)
		if(barriersInfo[i][O_TYPE] == 3 && rectanglesOverlap(r, barriers[i])) {
			if(!info[id]) {
...
void movePlayer(RECT &r1, RECT gameBoxWnd, int move, int &ok) {
 		if(GetAsyncKeyState(VK_LEFT) && !outOfGameBox(r1,gameBoxWnd, 0, move)) {
		...
		if(GetAsyncKeyState(VK_RIGHT) && !outOfGameBox(r1,gameBoxWnd, 1, move)) {
			r1.left += move;
			r1.right += move;
			ok = 2;
		}

		if(GetAsyncKeyState(VK_UP) && !outOfGameBox(r1,gameBoxWnd, 2, move)) {
			r1.top -= move;
			r1.bottom -= move;
			ok = 3;
		}
		...
		if(GetAsyncKeyState(VK_DOWN) && !outOfGameBox(r1,gameBoxWnd, 3, move)) {
			r1.top += move;
			r1.bottom += move;
			ok = 4;
		}
